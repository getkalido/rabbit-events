...
var _ = Describe("RabbitEvents", func() {
	...
	Describe("RabbitBatcher", func() {
		It("Should send messages after quiescence", func() {
			...
			var l sync.Mutex
			mockSender := func(ctx context.Context, m []string, x string, ri RabbitConfig, headers map[string]interface{}) {
				l.Lock()
				defer l.Unlock()
	...
	Describe("Exchange ReceiveFrom", func() {
		It("Should requeue the message once within a 10 seconds window if a processing error is encountered", func() {
			...
			go func() {
				handler(func(ctx context.Context, message []byte, headers map[string]interface{}) (err error) {
					replies <- struct{}{}
					doOnce <- struct{}{}
